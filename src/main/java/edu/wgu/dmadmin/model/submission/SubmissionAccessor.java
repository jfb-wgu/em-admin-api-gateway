...
public interface SubmissionAccessor {
    ...
    @Query("SELECT submission_id, student_id, status, attempt, evaluator_id FROM dm.submission_by_id WHERE submission_id = ?")
    ...
    @Query("SELECT * FROM dm.submission_attachment WHERE submission_id = ?")
    Result<SubmissionAttachmentModel> getAttachmentsForSubmission(UUID submissionId);
}
